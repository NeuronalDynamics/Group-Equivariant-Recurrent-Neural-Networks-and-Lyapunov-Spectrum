...
Lspectrum,LSall,pAll,LambdaMaxDNS,LambdaMaxDNSall = rate_Lyapunov_spectra(100,4,1e-1,1e3,100,1,1,1,1,True) #rate_Lyapunov_spectra(100,4,1e-1,1e2,2,0.1,1,1,1,True)
np.save("Lyapunov_spectrum.npy", Lspectrum)   # binary, loss-free
figure();semilogy(abs(LSall[:,0]-LambdaMaxDNSall),"k")
ylabel(r'$|\lambda^{local}_{DNS}- \lambda^{local}_{Jacobian}|$')
xlabel('steps')
show()
