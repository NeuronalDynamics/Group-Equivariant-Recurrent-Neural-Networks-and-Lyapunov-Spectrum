...
# python gpu_G-CAN_plus_speed_neurons_with_moving_bump_dynamical.py --speed 5
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
    ...
    N_STEP = int(T_SIM / DT)
    #h      = h_init.clone()
    # ---------- state now has an extra scalar φ(t) ------------------
    phi0   = torch.tensor([0.], dtype=DTYPE, device=DEVICE)   # initial phase
    h      = torch.cat((h_init, phi0))        # shape 3N+1
    state  = {}          # decoder memory
    traj   = []          # un-wrapped angles

    # ── Lyapunov-spectrum bookkeeping ───────────────────────────────
    #q, _ = qr(torch.randn(3*N, N_LE, generator=GEN_ONS, dtype=DTYPE))
    q, _ = qr(torch.randn(3*N + 1, N_LE, generator=GEN_ONS, dtype=DTYPE))
    Ls   = torch.zeros(N_LE, dtype=DTYPE)
    STEP_ONS = int(T_ONS / DT)
    ...
    def step_phi(h):
        ...
        # phase update  φ ← φ + v·Δt
        phi_new = phi + DT * v_cmd

        return torch.cat((u_s + DT*du_s,
                          u_p + DT*du_p,
                          u_m + DT*du_m,
                          phi_new.unsqueeze(0)))
    # compile if possible
    step_v = torch.compile(step_phi) \
            if (DEVICE.type=="cuda" and not args.no_compile and TRITON_AVAIL) \
            else step_phi
