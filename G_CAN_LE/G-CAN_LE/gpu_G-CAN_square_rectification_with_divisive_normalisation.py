...
# square-rectification with the divisive‐normalisation term
def G(u: torch.Tensor) -> torch.Tensor:
    up   = torch.clamp(u, min=0)        # ReLU
    num  = up.square()                  # or up**2
    # keep the denominator on the same device/dtype
    denom = 1 + k * rho_w * torch.sum(num)
    return num / denom
...
def f(h):
    """Euler step for translation-equivariant CAN"""
    return h*(1-DT) + DT * (rho_w * (Wmat @ G(h)))
