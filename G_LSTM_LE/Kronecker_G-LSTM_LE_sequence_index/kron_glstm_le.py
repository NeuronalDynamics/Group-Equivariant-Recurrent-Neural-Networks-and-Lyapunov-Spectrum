...
def main():
    ...
    ap.add_argument('--K', type=int, default=28, #4 8 16 28
                    help='channels per slot – hidden size is |G|·K')
    ap.add_argument('--trials', type=int, default=40)
    ap.add_argument('--device', default='cuda' if torch.cuda.is_available() else 'cpu')
    ap.add_argument('--gain', type=float, default=4.95) #4.95
    ...
    if args.calibrate:
        g_grid = np.arange(20.5, 25.5, 0.5)
        calibrate(g_grid,
                  device=dev,
