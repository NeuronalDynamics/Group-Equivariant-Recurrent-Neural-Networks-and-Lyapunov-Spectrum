# ---------------------------------------------------------------------
#  kronecker_g_lstm.py  –  Kronecker Representation of permutation-equivariant 1-layer G-LSTM for SMNIST
# ---------------------------------------------------------------------
...
# ---------------- Kronecker-sum G-LSTM *cell* ------------------------
#   Implements  Σ_r  P^r ⊗ A_r   acting on (B,|G|,K) tensors.
#   Equivariant by construction; taps=(0,1) reproduces your old δ-support cell.
class KronEqLinear(nn.Module):
    def __init__(self, K, out_features, perm, taps=(0,), bias=False):
        super().__init__()
        self.taps  = tuple(taps)
        # make `perm` a buffer so .to(device) moves it automatically
        self.register_buffer("perm", perm.clone())
        self.As    = nn.ModuleList([nn.Linear(K, out_features, bias=bias)
                                     for _ in self.taps])

    def _apply_perm(self, x, r: int):
        """
        Apply P^r to the slot dimension of (B,|G|,K) tensor `x`.
        Works for *any* permutation `self.perm`; no device copy needed
        because the buffer is already on the same device as `x`.
        """
        if r == 0:
            return x
        idx = self.perm                                # (|G|,) on same device
        # naive exponentiation – fine for |G|=28; avoids extra memory
        for _ in range(r - 1):
            idx = self.perm[idx]
        return x[:, idx]
...
class GLSTMCell(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.K     = K
        self.perm  = perm           # keep a reference if you need it later
        self.taps  = taps
    ...
    def forward(self, x, h, c):
        # x, h, c :  (B, |G|, K)
        gates = self.W_x(x) + self.W_h(h)           # (B, |G|, 4K)                              # (B,G,4K)
...
class GLSTMSMNIST(nn.Module):
    ...
    * Output   : 10-way classifier (same as before – unused for LE)
    ...
    def __init__(self, group_size: int = 28, K: int = 4, dropout: float = .1):
        ...
        # permutation generator  P : cyclic shift by +1
...
def critical_glstm_init(model: GLSTMSMNIST, *,
    ...
    if scheme == "gaussian":
        for layer in model.cell.modules():
            if isinstance(layer, nn.Linear):
                nn.init.normal_(layer.weight, 0.0, std)

    # keep derivatives alive: forget‑gate bias +1
    '''
    if model.cell.W_x.bias is not None:
        K = model.K
        for lin in model.cell.W_x.As + model.cell.W_h.As:
            lin.bias.data[K:2*K].fill_(1.0)
    '''
...
def main():
    ...
    ap.add_argument('--gain', type=float, default=8.45) #4.95
