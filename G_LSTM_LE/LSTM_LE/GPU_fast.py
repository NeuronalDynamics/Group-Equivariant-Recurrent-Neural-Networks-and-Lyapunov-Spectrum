...
def make_driver(batch=15, T=8_000, device='cpu', dtype=torch.float64):
...
def main():
    ...
    ap.add_argument('--hidden', type=int, default=224) #112
    ap.add_argument('--trials', type=int, default=40)
    ap.add_argument('--device', default='cuda' if torch.cuda.is_available() else 'cpu')
    ap.add_argument('--gain', type=float, default=4.05,
                help='initialisation gain g (W~U[-g/sqrt(H),g/sqrt(H)])') #4.95 edge of chaos #10.0 chaotics lazy high gain #2.0 ordered rich low gain
    ...
    for run in range(1, args.trials+1):
        ...
        g   = critical_lstm_init(net, g=args.gain, scheme="gaussian")
        all_g.append(g)   
